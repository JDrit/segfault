...
\textbf{Funny (Real) CS Quotes:}
\begin{itemize}[leftmargin=*]
\item "Beware of bugs in the above code; I have only proved it correct, not tried it."
- \textbf{Donald E. Knuth}
\item "Perl - The only language that looks the same before and after RSA encryption." - \textbf{Keith Bostic}
\item "There are two ways of constructing a software design. One way is to make it so simple that there are obviously no deficiencies. And the other way is to make it so complicated that there are no obvious deficiencies." - \textbf{C.A.R. Hoare}
